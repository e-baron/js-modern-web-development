  ...
  animation = anime({
    ...
    translateX: {
      value: function (target, index, targetsLength) {
        ...
        let randomX = anime.random(
          0,
          pageWidth - currentX - target.style.width
        );
    ...
    translateY: {
      value: function (target, index, targetsLength) {
        ...
        let randomY = anime.random(
          0,
          pageHeight - currentY - target.style.height
        );
...
allWords.forEach((item) => {
  item.addEventListener("click", (event) => {
    ...
    event.target.className = "word clicked d-inline-block";
